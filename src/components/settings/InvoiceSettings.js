...
const InvoiceSettings = ({ data, setData }) => {
  ...
  // This generic handler correctly updates the parent component's state.
  ...
  // This handler updates state when a new payment image file is selected.
  const handleImageChange = (e) => {
    const file = e.target.files?.[0];
    if (file) {
      // Store the actual File object in the parent's state for the upload process.
      setData(prevData => ({ ...prevData, paymentImageFile: file }));
      
      // Create a temporary URL for an instant preview for the user.
  ...
  // The definitive source for the preview image. It prioritizes the new local preview
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          ...
          <p className="text-xs text-gray-500 mt-1">The next invoice you create will be this number. e.g., {data?.invoicePrefix}{data?.nextInvoiceNumber}</p>
        </div>
      </div>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200">
        ...
        <div>
          ...
          <textarea 
            ...
            onChange={handleChange} // <-- FIX: Added the missing onChange handler
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <input 
                ...
                onChange={handleImageChange} // <-- FIX: Added the missing onChange handler
              />
              <label 
